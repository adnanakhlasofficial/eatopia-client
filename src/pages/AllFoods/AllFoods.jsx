...
const AllFoods = () => {
  ...
  const [sortMethod, setSortMethod] = useState("ASN");
  ...
  const handleSort = () => {
    ...
    const sortedFoods = foods.sort((prev, curr) => {
      if (sortMethod === "ASN") {
  ...
  return (
    <HelmetProvider>
      ...
      <div>
        ...
        <div className="wrapper flex justify-end gap-4">
          <select
            defaultValue={sortMethod}
            className="form-input !w-max !pr-2"
        ...
        <div className="wrapper grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 my-12 gap-6">
          {foods.map((food) => (
